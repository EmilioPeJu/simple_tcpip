...
void test_send_arp_broadcast_request()
{
...
void test_process_arp_reply_msg_adds_arp_entry()
{
    struct node *node0 = get_node_by_node_name(tgraph, "node0");
    struct intf *eth01 = get_node_if_by_name(node0, "eth01");
    ...
    TEST_ASSERT_MESSAGE(entry != NULL, "Arp entry not found");
}
...
void test_arp_table_add_and_lookup()
{
    ...
    memcpy(entry1.ip_addr.addr, ip1.addr, IP_ADDR_SIZE);
    memcpy(entry1.mac_addr.addr, "\x01\x02\x03\x04\x05\x06", MAC_ADDR_SIZE);
    entry1.intf = &intf1;
...
{
    UNITY_BEGIN();
    RUN_TEST(test_send_arp_broadcast_request);
    ...
    RUN_TEST(test_process_arp_reply_msg_adds_arp_entry);
